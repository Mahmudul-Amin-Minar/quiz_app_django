...
def quiz_data_view(request, myid):
    quiz = Quiz.objects.get(id=myid)
    questions = []
    quiz_q = list(quiz.get_questions())
    quiz_q = random.sample(quiz_q, quiz.number_of_questions)
    print(quiz_q)
    for q in quiz_q:
        answers = []
...
def show_leaderboard(request):
    quiz = request.GET.get('quiz_value') or None
    if quiz:
        marks = Marks_Of_User.objects.filter(quiz=quiz).order_by('-score')
    else:
        marks = Marks_Of_User.objects.all().order_by('-score')
    quiz = Quiz.objects.all()
    return render(request, "leaderboard.html", {'marks':marks, 'quiz':quiz})

@login_required(login_url = '/login')
def previous_tests(request):
    marks = Marks_Of_User.objects.filter(user=request.user).order_by('-taken_time')
    # quiz = Quiz.objects.all()
    for mark in marks:
        print(mark.taken_time)
    return render(request, "previous_test.html", {'marks':marks,})
